	...
	<div class="page_content_wrapper">
		...
		<div class="singleroom_other_rooms">
			...
			?>
					...
					<div class="room_grid_link_wrapper">
						<a title="<?php echo esc_attr($room_type->post_title); ?>" href="<?php echo esc_url(get_permalink($room_type->ID)); ?>" class="room_grid_book">
							<?php esc_html_e('Book Now From', 'hoteller' ); ?>&nbsp;<?php mphb_tmpl_the_room_type_default_price($room_type->ID); ?>
						</a>
					</div>
				</div>
			<?php
			...
			?>
					...
					<?php
						if(!empty($room_type->post_content))
						{
					?>
						<div class="room_grid_content_wrapper themeborder">
							<?php echo hoteller_substr(strip_tags(strip_shortcodes($room_type->post_content)), 150); ?>
						</div>
					<?php
						}
					?>
					<div class="room_grid2_action_wrapper">
						<div class="child_one_half themeborder">
							...
							<div class="room_grid2_price"><?php mphb_tmpl_the_room_type_default_price($room_type->ID); ?></div>
						</div>
