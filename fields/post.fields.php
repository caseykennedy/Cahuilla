...
function rt_add_gravatar_col($cols) {
	$cols['thumbnail'] = esc_html__('Thumbnail', 'hoteller');
	return $cols;
}
...
$postmetas = 
	array (
		'post' => array(
			array("id" => "post_layout", "type" => "select", "title" => esc_html__('Post Layout', 'hoteller' ), "description" => esc_html__('You can select layout of this single post page.', 'hoteller' ), "items" => $post_layout_select),
			
			array("id" => "page_menu_transparent", "type" => "checkbox", "title" => esc_html__('Make Menu Transparent', 'hoteller' ), "description" => esc_html__('Check this option if you want to display main menu in transparent', 'hoteller' )),
			
			array("id" => "post_video_embed", "type" => "textarea", "title" => esc_html__('Video Embed Code', 'hoteller' ), "description" => esc_html__('Insert Youtube or Vimeo embed code.', 'hoteller' )),
		),
		
		'mphb_room_type' => array(	
			array("id" => "custom_booking_url", "type" => "text", "title" => esc_html__('Custom Booking URL (Optional)', 'hoteller' ), "description" => esc_html__('Enter URL for booking option so when user click on book button. It will opens this entered URL. Recommend if you are using 3rd party booking system.', 'hoteller' )),
		),
);
...
function hoteller_create_meta_box() {
	...
	if ( function_exists('add_meta_box') && isset($postmetas) && count($postmetas) > 0 ) {  
		foreach($postmetas as $key => $postmeta)
		{
			...
			{
				add_meta_box( 'metabox', esc_html__("Options", 'hoteller'), 'hoteller_new_meta_box', $key, 'normal', 'high' );
			}
		}
	}
		...
		if($_GET['post_type'] == $key)
		{
			...
			foreach ( $postmeta as $postmeta_key => $each_meta ) {
				...
				if ($meta_type == 'checkbox') {
					...
					echo '<th width="5%"></th>';
					echo '<th width="90%">'.esc_html__("Title", 'hoteller').'</th>';
					echo '<th width="5%"></th>';
					...
					echo '<a id="'.$meta_id.'_add_row" class="button adding_list_add_row" data-target="'.$meta_id.'_sortable" data-metaid="'.$meta_id.'" href="javascript:;">'.esc_html__("Add another", 'hoteller').'</a>';
